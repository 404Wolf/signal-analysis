...
export const getMessages = async (
  ...
  return (query.all() as inMessage[]).map(
    (m: inMessage) =>
      ({ name: m.profile_joined_name, body: m.body }) as Message,
  );
};

export const rollMessages = async (
  messages: Message[],
  windowSize = 5,
): Promise<Message[][]> => {
...
export const formatRoll = async (message: Message[]): Promise<string> => {
